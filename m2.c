/**
 * File Server-Client Architecture for File Manipulation and Searching
 * This program implements a server that listens on a specified port and handles client connections.
 * The server supports various file-related commands requested by clients.
 *
 * Main Components and Functionalities:
 *
 * 1. Header Inclusions:
 *    - Necessary header files for system functions, socket handling, file operations, and string manipulation.
 *
 * 2. Macro Definitions:
 ...
 *
 * 3. Global Variables:
 *    - path: A character array to store the path of the current working directory.
 *
 * 4. Function Prototypes:
 *    - processclient(int socket): Handles client communication and command execution.
 *    - mirrorRedirection(int socket): Handles redirection for overloaded clients to a mirror server.
 *    - validDate(const char *date): Validates a date string in the "YYYY-MM-DD" format.
 *    - execute_tarfgetz_command(int sockfd, char *size1_str, char *size2_str, char *uZipFlag, char *command):
 *      Executes the "tarfgetz" command to create a tar file containing files within a size range.
 *    - execute_getdirf_command(int sockfd, char *date1_str, char *date2_str, char *uZipFlag, char *command):
 *      Executes the "getdirf" command to create a tar file containing files within a date range.
 *    - execute_fgets_command(int sockfd, const char *args): Executes the "fgets" command to create a tar file
 *      containing files with specified names.
 *    - execute_targzf_command(int sockfd, char *extensions[]): Executes the "targzf" command to create a tar file
 *      containing files with specified extensions.
 *    - searchFile(const char *filename, const char *currentPath, char *result): Recursively searches for a file
 *      within a directory hierarchy and provides details.
 *    - execute_filesrch_command(const char *filename, int sockfd): Executes the "filesrch" command to search for a file.
 *    - quit(int sockfd): Closes the client connection and exits the child process.
 *
 * 5. Main Function:
 ...
 *
 * Note: The code uses socket programming, forking processes, and system commands to perform file-related operations
...
void searchFile(const char *filename, const char *currentPath, char *result)
{
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        {
            searchFile(filename, filePath, result);         }
